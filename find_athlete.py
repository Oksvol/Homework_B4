...
def request_data():
    user_id = input("Введите идентификатор пользователя для поиска похожих атлетов: ")
    return int(user_id)
...
def main():
    ...
    if not user:
        print("Атлет не найден")
    else:
        bd_athlete, bd = nearest_by_bd(user, session)
        height_athlete, height = nearest_by_height(user, session)
        print(
            "Ближайший атлет по дате рождения: {}, дата рождения: {}".format(bd_athlete, bd)
        )
        print(
            "Ближайший атлет по росту: {}, рост: {}".format(height_athlete, height)
        )
